...
export const envValidationSchema = Joi.object({
  ...
  JWT_ACCESS_SECRET: Joi.string().min(16).when('JWT_SECRET', { is: Joi.exist(), otherwise: Joi.required() }),
  JWT_REFRESH_SECRET: Joi.string().min(16).when('JWT_SECRET', { is: Joi.exist(), otherwise: Joi.required() }),
